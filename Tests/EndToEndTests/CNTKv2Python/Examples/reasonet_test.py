...
def test_reasonet(device_id):
  ...
  (train_loss, train_acc, eval_acc) = rsn.train(model, train_data, max_epochs=max_epochs, epoch_size=epoch_size, save_model_flag=False, model_name=os.path.basename(data_path), eval_data=eval_data, eval_size=eval_size, check_point_freq=1)
